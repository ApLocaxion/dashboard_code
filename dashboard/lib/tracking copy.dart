...
final List<ScrapBin> mockBins = List.generate(25, (index) {
  ...
  return ScrapBin(
    ...
    weightLbs: (rnd.nextInt(2400) + 100),
    zone: location,
...
// Helper to find bin by ID (simulated DB lookup)
ScrapBin findBinById(String id) {
  return mockBins.firstWhere(
    (b) => b.id == id,
    orElse: () => ScrapBin(
      id: id,
      alloy: 'Unknown Alloy',
...
class _ScanScreenState extends State<ScanScreen> {
  ...
  void _handleSubmit(String value) {
    ...
    final bin = findBinById(value.toUpperCase());
    widget.onScan(bin);
    _controller.clear();
...
// --- CONFIRM SCREEN (UPDATED FOR REAL SCANNER) ---
class ConfirmScreen extends StatefulWidget {
  final ScrapBin initialBin;
  final String currentZone;
  final Function(ScrapBin) onConfirm;
...
class _ConfirmScreenState extends State<ConfirmScreen> {
  late ScrapBin _currentBin;
  // Focus node to capture USB scanner input
  ...
  void initState() {
    super.initState();
    _currentBin = widget.initialBin;
    // Automatically focus the hidden field to capture hardware scans
  ...
  void _handleHardwareScan(String scannedValue) {
    if (scannedValue.isEmpty) return;

    // 1. Lookup the bin details
    final newBin = findBinById(scannedValue.toUpperCase());

    // 2. Update UI only if different (Filtering/Debouncing)
    if (newBin.id != _currentBin.id) {
      setState(() {
        _currentBin = newBin;
      });
    }

    // 3. Clear controller for next scan and keep focus
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        // HIDDEN TEXT FIELD FOR USB SCANNER
        // This captures the keyboard input from the scanner without showing a UI element
        Opacity(
          opacity: 0,
          child: TextField(
            ...
            autofocus: true,
            onSubmitted: _handleHardwareScan,
          ),
        ),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // Updates dynamically on scan
                  AnimatedSwitcher(
                    duration: const Duration(milliseconds: 300),
                    child: Text(
                      _currentBin.id,
                      key: ValueKey(_currentBin.id),
                      style: const TextStyle(
                        fontSize: 90,
                        fontWeight: FontWeight.w900,
                        color: AppColors.primaryBlue,
                  ...
                  DashboardCard(
                    ...
                    child: Column(
                      children: [
                        _DetailRow(label: 'ALLOY', value: _currentBin.alloy),
                        const Padding(
                        ...
                        _DetailRow(
                          label: 'WEIGHT',
                          value: '${_currentBin.weightLbs} lbs',
                        ),
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 20),
                          child: Divider(height: 1, color: AppColors.border),
                        ),
                        _DetailRow(label: 'BIN ZONE', value: _currentBin.zone),
                      ],
                    ),
                  ),
                  const SizedBox(height: 40),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: ActionButton(
                          ...
                          label: 'YES',
                          isPrimary: true,
                          // Confirms the CURRENTLY displayed bin
                          onTap: () => widget.onConfirm(_currentBin),
                        ),
                      ),
                    ],
                  ),
        ...
        Positioned(
          top: 0,
          right: 0,
          child: ZoneBadge(zone: widget.currentZone),
        ),
      ],
    );
  }
...
class _BinReportScreenState extends State<BinReportScreen> {
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        _buildHeaderCell('ORIGIN', flex: 3),
                      ],
                    ),
                  ),
  ...
  String _calculateTotalWeight() {
    ...
    if (_showKg) return '${(totalLbs * 0.453592).round()} kg';
    return '$totalLbs lbs';
  }
